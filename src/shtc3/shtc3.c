...
esp_err_t wakeup_sensor(uint8_t addr)
{
    ...
    i2c_master_stop(cmd);
    esp_err_t err = i2c_master_cmd_begin(I2C_NUM_0, cmd, 20);
    i2c_cmd_link_delete(cmd);

    if(err != ESP_OK)
        ESP_LOGI(TAG, "wakeup %s", esp_err_to_name(err));
...
esp_err_t read_out(uint8_t addr, uint16_t read_mode, float *temp, float *hum)
{
    ...
    i2c_master_stop(cmd);
    esp_err_t err = i2c_master_cmd_begin(I2C_NUM_0, cmd, 5);
    if(err != ESP_OK){
        ESP_LOGI(TAG, "read_out %s", esp_err_to_name(err));
        return err;
    }
